...
class WasteExtraction:
    ...
    def get_waste_frame(self, fiftyFrame, model, leftbbox, rightbbox, filename, grabberXArray, grabberStartX):
        print("We closing!")
        ...
        for result in yoloResults:
            if len(result.boxes) > 0:
                ...
                if grabberXArrayminus40[frameNumber] - 40 > grabberStartX:
                    ...
                    frameNumber += 1
                    continue
                if int(result.boxes.xyxy[0][0]) > screenSize and int(result.boxes.xyxy[0][2]) < fiftyFrame[0].shape[
                    1] - screenSize and int(result.boxes.xyxy[0][3]) < fiftyFrame[0].shape[0] - 80 and int(
                        result.boxes.xyxy[0][1]) > 80:
